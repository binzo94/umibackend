    ...
    Http.get('/system/resource/open/system')
      .then(res => {
        if (res.data.code==0) {
          ...
          var defaultSystem = res.data.content.filter(item => item.id === '831b3b00-0393-491b-a2e1-196ca2f9e0f8')
    ...
    return (
      <React.Fragment>
        <div className={'container-left'}>
          {data && <SideBar data={data} baseUrl="/user-admin" />}
        </div>
