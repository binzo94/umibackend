...
class Layout extends React.Component {
  ...
  getSystem = () => {
    Http.get('/system/resource/open/system')
      .then(res => {
        if (res.data.code==0) {
          var defaultSystem = res.data.content.filter(item => item.id === '9bd284fb-ac12-4516-881e-f176df65020a')
    ...
    return (
      <React.Fragment>
        ...
        <div className={'container-left'}>
          {data && <SideBar data={data} baseUrl="/internal-admin" />}
        </div>
